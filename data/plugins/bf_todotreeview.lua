...
}, config.plugins.bf_todotreeview)
...
local function is_file_ignored(filename)
  for _, path in ipairs(config.plugins.bf_todotreeview.ignore_paths) do
...
function TodoTreeView:is_file_in_scope(filename)
  if config.plugins.bf_todotreeview.todo_scope == SCOPES.ALL then
    return true
  elseif config.plugins.bf_todotreeview.todo_scope == SCOPES.FOCUSED then
    ...
    assert(false, "Unknown scope defined ("..config.plugins.bf_todotreeview.todo_scope..")")
  end
end
...
-- run cargo check and get all warnings/errors
function TodoTreeView.get_all_cargo_warnings() 
  -- fetch all warnings 
  local cargo_check_stdout = exec({"cargo", "check", "--quiet", "--message-format=json-diagnostic-short"})

  -- result object with warnings for each file
  local result_files_with_warnings = {}

  -- return if no problems found
  if not cargo_check_stdout or #cargo_check_stdout == 0 then
    return result_files_with_warnings
  end
  ...
  for line in string.gmatch(cargo_check_stdout, "([^\n]+)") do
    ...
    -- check if json object is a compiler message
    if parsed_json["reason"] == "compiler-message" then
      ...
      -- only proceed if array actually has items
      if #arr_locations > 0 then
        -- simply pick first item to get file name and line number
        local error_in_file_relative_path = arr_locations[0]["file_name"]
        local error_in_line = arr_locations[0]["line_start"]

        -- add to result object, but only once
        if not warnings_for_file[relative_path_to_file] then
          warnings_for_file[relative_path_to_file] = {
            error_level = level,
            error_message = message,
            error_in_file_relative_path = error_in_file_relative_path,
...
function TodoTreeView:refresh_cache()
  ...
  core.add_thread(function()
    ...
    if config.plugins.bf_todotreeview.todo_only_files_from_git_repository then
    ...
    for _, item in pairs(self.get_all_files()) do
      ...
      if not ignored 
        and config.plugins.bf_todotreeview.todo_only_files_from_git_repository
      then
      ...
      if not ignored and item.type == "file" then
        ...
        elseif config.plugins.bf_todotreeview.todo_mode == "file_tag" then
          local file_t = {}
          file_t.expanded = config.plugins.bf_todotreeview.todo_expanded
          file_t.type = "file"
          file_t.tags = {}
          file_t.todos = {}
          ...
          for _, todo in ipairs(cached.todos) do
            ...
            if not file_t.tags[tag] then
              local tag_t = {}
              tag_t.expanded = config.plugins.bf_todotreeview.todo_expanded
              tag_t.type = "group"
              tag_t.todos = {}
              tag_t.tag = tag
          ...
          for _, todo in ipairs(cached.todos) do
            ...
            if not items[tag] then
              local t = {}
              t.expanded = config.plugins.bf_todotreeview.todo_expanded
              t.type = "group"
              t.todos = {}
              t.tag = tag
              items[tag] = t
    ...
    if config.plugins.bf_todotreeview.todo_mode == "tag" and next(self.items) then
    ...
    -- -- if this is a rust project, run cargo check 
    -- for filename, error in pairs(self.get_all_cargo_warnings()) do
    --   -- if TODO entry exists for this file, then add to this todo
    --   if items[error_level] then
...
local function find_file_todos(t, filename)
  ...
  for line in fp:lines() do
    for _, todo_tag in ipairs(config.plugins.bf_todotreeview.todo_tags) do
      ...
      if s then
        ...
        if d.text == "" then
          d.text = config.plugins.bf_todotreeview.todo_default_text
        end
        d.line = n
        d.col = s
        table.insert(t, d)
...
function TodoTreeView:set_target_size(axis, value)
  if axis == "x" then
    config.plugins.bf_todotreeview.treeview_size = value
    return true
  end
end
...
function TodoTreeView:update()
  ...
  -- update width
  local dest = self.visible and config.plugins.bf_todotreeview.treeview_size or 0
...
function TodoTreeView:draw()
  ...
  for item, x,y,w,h in self:each_item() do
    ...
    local file_color = config.plugins.bf_todotreeview.todo_file_color.name or style.text
    if config.plugins.bf_todotreeview.tag_colors[item.tag] then
      text_color = config.plugins.bf_todotreeview.tag_colors[item.tag].text or style.text
      tag_color = config.plugins.bf_todotreeview.tag_colors[item.tag].tag or style.text
    end
    ...
    if item == self.hovered_item then
      ...
      file_color = config.plugins.bf_todotreeview.todo_file_color.hover or style.accent
      if config.plugins.bf_todotreeview.tag_colors[item.tag] then
        text_color = config.plugins.bf_todotreeview.tag_colors[item.tag].text_hover or style.accent
        tag_color = config.plugins.bf_todotreeview.tag_colors[item.tag].tag_hover or style.accent
      end
    end
    ...
    elseif item.type == "group" then
      ...
      if config.plugins.bf_todotreeview.todo_mode == "tag" then
        x = x + style.padding.x
      else
...
view.size.x = config.plugins.bf_todotreeview.treeview_size
