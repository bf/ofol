...
function lsp.get_hovered_location(x, y)
  ...
  if av and av.doc.lsp_open then
    ...
    if x > last_x + lx or y > ly + av:get_editor_line_height() then return end
    return doc, line, col
  end
end
  ...
  for _, name in pairs(lsp.get_active_servers(doc.filename, true)) do
    ...
    if server.capabilities.documentFormattingProvider then
      ...
      if not indent_confirmed then
        indent_type = ConfigurationOptionStore.get_editor_tab_type()
        indent_size = ConfigurationOptionStore.get_editor_indent_size()
      end
...
core.status_view:add_item({
  ...
  get_item = function()
    ...
    if #diagnostics.list > 0 then
      ...
      if lowest_severity_number == 1 then
        icon_color = style.error
      elseif lowest_severity_number == 2 then
        icon_color = style.warn
      end
      ...
      return {
        icon_color,
        style.icon_font, "!",
        style.font, " " .. tostring(#diagnostics.list)
      }
    end
