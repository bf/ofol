...
local commands = {
  ...
  ["doc:newline"] = function(dv)
    for idx, line, col in dv.doc:get_selections(false, true) do
      ...
      if not ConfigurationOptionStore.get_keep_newline_whitespace() and dv.doc.lines[line]:match("^%s+$") then
...
command.add(function(x, y)
  if x == nil or y == nil or not (core.active_view:extends(DocView) or core.active_view:extends(SingleLineTextView)) then 
    return false 
  end
