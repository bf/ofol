...
function ContextMenu:show(x, y)
  ...
  if #items_list > 0 then
    ...
    x = math.clamp(x, 0, core.root_view.size.x - w - style.padding.x)
    y = math.clamp(y, 0, core.root_view.size.y - h)
...
function ContextMenu:draw_context_menu()
  ...
  for i, item, x, y, w, h in self:each_item() do
    if item == DIVIDER then
      ...
      renderer.draw_text_aligned_in_box(style.font, style.text, item.text, "left", x + style.padding.x, y, w, h)
      if item.info then
        renderer.draw_text_aligned_in_box(style.font, style.dim, item.info, "right", x, y, w - style.padding.x, h)
      end
    end
  end
end
